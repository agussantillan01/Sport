...
@APELLIDO VARCHAR (100)
	...
	IF @cantidadUsuariosRegistrados = 0 
		Insert into USUARIOS (NOMBRE,APELLIDO ,EMAIL, CONTRASEÑA, TipoUser) VALUES (@NOMBRE,@APELLIDO,@EMAIL, @PASSWORD, 2)
	ELSE 
	Insert into USUARIOS (NOMBRE,APELLIDO ,EMAIL, CONTRASEÑA, TipoUser) VALUES (@NOMBRE,@APELLIDO,@EMAIL, @PASSWORD, 1)
END

GO

ALTER PROCEDURE SP_AGREGARACTIVIDAD (
@IDPROFESOR INT, 
...
GO 
ALTER PROCEDURE SP_AGREGAACTIVIDADXUSUARIO (
@ID INT, 
@IDACTIVIDAD INT
)AS BEGIN 
	INSERT INTO USUARIOS_ACTIVIDAD (IDUSUARIO, IDACTIVIDAD) VALUES (@ID, @IDACTIVIDAD)
END
...
SELECT U.ID AS IDUSUARIO, U.NOMBRE AS NOMBREUSUARIO,U.APELLIDO as APELLIDOUSUARIO,U.DNI as DNIUSUARIO, U.EMAIL AS EMAILUSUARIO, U.CONTRASEÑA as USUARIOCONTRASEÑA, U.FECHANACIMIENTO as USUARIOFECHANACIMIENTO,ACT.ID AS IDACTIVIDAD ,ACT.ACTIVIDAD as NOMBREACTIVIDAD, ACT.DIAS AS DIASACTIVIDADES,ACT.MODALIDAD AS IDMODALIDAD,M.MODALIDAD AS NOMBREMODALIDAD,ACT.FECHAINICIO AS FECHAINICIO, ACT.FECHAFIN AS FECHAFIN, ACT.HORARIOINICIO,ACT.HORARIOFIN, ACT.PRECIO,ACT.IMAGEN FROM USUARIOS_ACTIVIDAD UXA INNER JOIN USUARIOS U ON U.ID=UXA.IDUSUARIO INNER JOIN ACTIVIDADES ACT ON ACT.ID= UXA.IDACTIVIDAD INNER JOIN MODALIDAD M ON M.ID=ACT.MODALIDAD WHERE UXA.IDUSUARIO=@IDUSUARIO
END
